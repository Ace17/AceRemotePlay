...
AVFrame* convertAudioFrame(OutputStream* os, AVFrame* frame)
{
  AVCodecContext* c = os->enc;
  ...
  os->converted_frame->pts = av_rescale_q(os->sampleCount, (AVRational) { 1, c->sample_rate }, c->time_base);
...
void writeOneAudioFrame(AVFormatContext* oc, OutputStream* os)
{
  ...
  frame = convertAudioFrame(os, frame);

  int got_packet;
  int ret = avcodec_encode_audio2(os->enc, &pkt, frame, &got_packet);

  if(ret < 0)
    throw std::runtime_error(std::string("Error encoding audio frame: ") + averrorToString(ret));

  if(got_packet)
    write_frame(oc, &os->enc->time_base, os->st, &pkt);
}
...
void open_video(AVCodec* codec, OutputStream* os)
{
  ...
  c->max_b_frames = 0;
  c->gop_size = 25;
  safe_av_opt_set(c->priv_data, "profile", "main", 0);
  safe_av_opt_set(c->priv_data, "tune", "zerolatency", 0);

  if(0) // horrible VQ
    safe_av_opt_set(c, "bufsize", "1000k", 0);
  if(0) // horrible VQ
    safe_av_opt_set(c->priv_data, "x264opts", "intra-refresh=1", 0);

  int ret = avcodec_open2(c, codec, nullptr);

  if(ret < 0)
    throw std::runtime_error("Could not open video codec: " + averrorToString(ret));
...
void writeOneVideoFrame(AVFormatContext* oc, OutputStream* os)
{
  ...
  if(got_packet)
  {
    if(0)
    {
      static FILE* fp = fopen("frames.csv", "wb");
      fprintf(fp, "%d\n", (int)pkt.size);
    }
...
struct ISink
{
  virtual void pushPacket(const uint8_t* data, int len) = 0;
};

struct Streamer
{
public:
  Streamer(Size2i resolution, ISink* sink)
    : m_sink(sink)
  {
    ...
    oc->pb = avio_alloc_context((unsigned char*)av_malloc(SIZE), SIZE, 1, this, nullptr, &Streamer::writePackets, nullptr);
...
private:
  ...
  std::queue<Packet> outputFifo;
  int bytesBudget = 0;

  static int writePackets(void* opaque, uint8_t* data, int len)
  {
    return ((Streamer*)opaque)->writePackets((const uint8_t*)data, len);
  }

  int writePackets(const uint8_t* data, int len)
  {
    assert(len % 188 == 0);
    ...
    {
      outputFifo.push({});
      memcpy(outputFifo.back().data, data + i, 188);
    }

    return len;
  }

  void processOutput(int elapsedMs)
  {
    bytesBudget += 1000000LL * elapsedMs / 1000;
    bytesBudget = std::min(bytesBudget, 188 * 50);

    if(outputFifo.size() >= 500)
      printf("Warning: output fifo size: %d\n", (int)outputFifo.size());

    while(!outputFifo.empty() && bytesBudget > 0)
    {
      auto& pkt = outputFifo.front();
      m_sink->pushPacket(pkt.data, sizeof pkt.data);
      outputFifo.pop();
      bytesBudget -= 188;
    }
  }
...
struct NetClient
{
  NetClient(Address address_) : m_address(address_)
  {
...
struct StreamingServer : IStreamingServer, private ISink
{
  ...
  bool processOnePacket()
  {
    ...
    if(m_clients.find(addr) == m_clients.end())
    {
      ...
      m_clients[addr] = std::make_unique<NetClient>(addr);
    }
  ...
  void serverThread()
  {
    Streamer streamer(m_resolution, this);
...
private:
  ...
  void pushPacket(const uint8_t* data, int len) override
  {
    for(auto& client : m_clients)
      m_sock.send(client.first, data, len);
  }
  ...
  std::map<Address, std::unique_ptr<NetClient>, AddressLess> m_clients;
};
}
